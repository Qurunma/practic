 ...
 * @returns {{ FIO: string, social: object, date: object }} Объект дела
 */
function makeTodoItemFromData(data) {
  ...
  if (!todoItem.FIO) errors.push({ field: "FIO", message: "Не указан FIO" });
  if (!todoItem.social)
    errors.push({ field: "social", message: "Не указан social" });
  if (!todoItem.date) errors.push({ field: "date", message: "Не указан date" });
 ...
 * @returns {{ FIO: string, social: object, date: object }[]} Массив дел
 */
 ...
 * @param {{ FIO?: string, social?: object, date?: object }} data - Объект с изменяемыми данными
 * @throws {TodoApiError} Дело с таким ID не найдено (statusCode 404)
 * @throws {TodoApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ name: string, owner: string, done?: boolean }} Объект дела
 */
...
createServer(async (req, res) => {
  ...
  .on("listening", () => {
    console.log(
      `Сервер TODO запущен. Вы можете использовать его по адресу http://localhost:${PORT}`
    );
